...
```bash
git clone https://github.com/2BitwiseBard/rocky ~/Development/rocky && cd ~/Development/rocky
...
Git is the source of truth (since 2026-09-22; the zip/drop workflow is
retired). Work on a branch or on main, run the fast suites, commit; the
notebook (`BUILD_LOG.md`) and the raw inbox (`NOTES_INBOX.md`) are the
record, and `docs/SIM_GUIDE.md` / `docs/RL_GUIDE.md` are the how-tos.
