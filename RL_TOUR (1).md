...
6. **Sigma-anneal experiment** (a real open item): add a linear decay of
   ...
   plateau at ~224 move?
7. **Fix recovery's last gap.** The policy rights the body but props on
   ~1.5 feet; the hybrid handoff stands it up 12/20. Change RecoverEnv's
   success criterion to the HANDOFF criterion (upright + high for 0.5 s)
   or add +0.2·feet_down when upright — retrain, re-eval with
   `eval_recover.py`. Beating 12/20 is a real contribution to the robot.
