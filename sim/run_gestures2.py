...
fell or lifted a planted foot for >0.3 s.
"""
import json
import os
import subprocess
...
from pebble_gestures2 import GESTURES2, WAVE_ARM                     # noqa: E402
...
def run_one(name, video=True):
    ...
    ok = (not fell) and (gaited or break_time < 0.3)
    res["ok"] = bool(ok)
    print(f"{name:14s} tilt_max {tilt_max:5.2f}°  sag {res['height_sag_mm']:6.1f} mm  "
          ...
          f"disp {disp[0]:+6.0f},{disp[1]:+6.0f} mm  {'FELL' if fell else ('OK' if ok else 'LIFTS')}",
          flush=True)
    if video:
...
def main():
    ...
    results = [run_one(n, video=video) for n in names]
